...
public class FHIRValueSetCodeValidationService implements TxResourceAware {
	...
	public Parameters validate(FHIRCodeValidationRequest request) {
		...
		CodeSystemVersionProvider codeSystemVersionProvider = buildCodeSystemVersionProvider(codings, systemVersionHints, request);
		...
		try{
			...
			return handleConstraintGenerationException(e, request, hapiValueSet, systemVersionHints);
		}

		List<OperationOutcome.OperationOutcomeIssueComponent> versionCheckIssues = codeSystemVersionProvider.getVersionCheckIssues();

		codings = applyInferSystem(codings, request, codeSelectionCriteria);

		CodeSystemVersionMatch versionMatch = resolveMatchingCodeSystemVersions(codings, codeSelectionCriteria);
		Set<FHIRCodeSystemVersion> resolvedCodeSystemVersionsMatchingCodings = versionMatch.resolvedVersions();
		boolean systemMatch = versionMatch.systemMatch();
		...
		if (resolvedCodeSystemVersionsMatchingCodings.isEmpty()) {
			return buildNoMatchingVersionResponse(response, systemMatch, codings, codingA, codeSelectionCriteria, request, codeSystemVersionProvider, systemVersionHints, hapiValueSet, versionCheckIssues);
		}
		...
		applyVersionCheckIssues(versionCheckIssues, response, issues, request);
	...
	private record CodeSystemVersionMatch(Set<FHIRCodeSystemVersion> resolvedVersions, boolean systemMatch) {}

	private CodeSystemVersionProvider buildCodeSystemVersionProvider(List<Coding> codings, Set<CanonicalUri> systemVersionHints, FHIRCodeValidationRequest request) {
		...
		boolean allCodingVersionsUnknownToServer = !codingSystemVersionsWithVersion.isEmpty() &&
				codingSystemVersionsWithVersion.stream().noneMatch(this::codeSystemVersionExistsOnServer);
		...
		return new CodeSystemVersionProvider(systemVersionParamForProvider, codingSystemVersionsWithVersion, request.getCheckSystemVersion(), request.getForceSystemVersion(), null, allowCheckSystemVersionAsFallback, codeSystemService);
	}

	private List<Coding> applyInferSystem(List<Coding> codings, FHIRCodeValidationRequest request, CodeSelectionCriteria codeSelectionCriteria) {
		if (request.getInferSystem() != null && request.getInferSystem().booleanValue()) {
			...
			return codings.stream().flatMap(c -> {
	...
	private CodeSystemVersionMatch resolveMatchingCodeSystemVersions(List<Coding> codings, CodeSelectionCriteria codeSelectionCriteria) {
		...
		return new CodeSystemVersionMatch(resolvedCodeSystemVersionsMatchingCodings, systemMatch);
	}

	private void applyVersionCheckIssues(List<OperationOutcome.OperationOutcomeIssueComponent> versionCheckIssues, Parameters response, List<OperationOutcome.OperationOutcomeIssueComponent> issues, FHIRCodeValidationRequest request) {
	...
	private Parameters handleConstraintGenerationException(SnowstormFHIRServerResponseException e, FHIRCodeValidationRequest request, ValueSet hapiValueSet, Set<CanonicalUri> systemVersionHints) {
		if (OperationOutcome.IssueType.INVARIANT.equals(e.getIssueCode())
				&& hasIssueWithExtension(e, OperationOutcome.IssueType.INVARIANT, MISSING_VALUESET)) {
			return buildValueSetDefinitionNotFoundResponse(OperationOutcome.IssueType.INVARIANT, e, request, hapiValueSet);
		} else if (OperationOutcome.IssueType.NOTFOUND.equals(e.getIssueCode())
				&& hasIssueWithExtension(e, OperationOutcome.IssueType.NOTFOUND, "https://github.com/IHTSDO/snowstorm/available-codesystem-version")) {
			return buildUnknownCodeSystemVersionResponse(e, request, hapiValueSet, systemVersionHints);
		} else if (OperationOutcome.IssueType.NOTFOUND.equals(e.getIssueCode())
				&& hasIssueWithExtension(e, OperationOutcome.IssueType.NOTFOUND, MISSING_VALUESET)) {
			return buildValueSetDefinitionNotFoundResponse(OperationOutcome.IssueType.NOTFOUND, e, request, hapiValueSet);
		} else {
			throw e;
		}
	}

	private boolean hasIssueWithExtension(SnowstormFHIRServerResponseException e, OperationOutcome.IssueType issueType, String extensionUrl) {
		return !e.getOperationOutcome().getIssue().stream()
				.filter(i -> issueType.equals(i.getCode()))
				.flatMap(ex -> ex.getExtension().stream())
				.filter(ex -> ex.getUrl().equals(extensionUrl))
				.toList().isEmpty();
	}

	// Shared builder for the two near-identical "value set definition could not be found" responses
	// (one raised with an INVARIANT issue code, the other with NOTFOUND).
	private Parameters buildValueSetDefinitionNotFoundResponse(OperationOutcome.IssueType issueCode, SnowstormFHIRServerResponseException e, FHIRCodeValidationRequest request, ValueSet hapiValueSet) {
		String valueSetCanonical = e.getOperationOutcome().getIssue().stream().filter(i -> issueCode.equals(i.getCode())).flatMap( ex -> ex.getExtension().stream()).filter(ex -> ex.getUrl().equals(MISSING_VALUESET)).map(ext -> ext.getValue().primitiveValue()).findFirst().orElse(null);
		Parameters response = new Parameters();
		if (request.getCodeableConcept()!=null){
	...
	private Parameters buildUnknownCodeSystemVersionResponse(SnowstormFHIRServerResponseException e, FHIRCodeValidationRequest request, ValueSet hapiValueSet, Set<CanonicalUri> systemVersionHints) {
		Parameters response = new Parameters();
		// Get the code being validated
		String theCode = resolveValidatedCode(request);
		...
		String requestedSystem = resolveRequestedSystem(request, missing);
		...
		if (valueSetSystemMatchesRequestedSystem) {
			return buildUnknownVersionMatchedSystemResponse(response, e, request, hapiValueSet, systemVersionHints, theCode, missing, availableVersion, requestedSystem);
		}
		return buildCodeSystemNotFoundResponse(response, hapiValueSet, requestedSystem, theCode, missing);
	}

	private Parameters buildUnknownVersionMatchedSystemResponse(Parameters response, SnowstormFHIRServerResponseException e, FHIRCodeValidationRequest request, ValueSet hapiValueSet, Set<CanonicalUri> systemVersionHints, String theCode, CanonicalUri missing, String availableVersion, String requestedSystem) {
		...
		String codingVersion = resolveCodingVersion(request);
		...
		FallbackResolution fallbackResolution = resolveDisplayFallbackVersion(isVersionlessInclude, codingVersion, requestedSystem, systemVersionHints, request);
		String hintVersion = fallbackResolution.hintVersion();
		FHIRCodeSystemVersion fallback = fallbackResolution.fallback();
		...
		UnknownVersionIssues unknownVersionIssues = buildUnknownVersionIssues(isVersionlessInclude, codingVersion, requestedSystem, originalIncludeVersion, hintVersion, fallback, unknownMsgText, unknownIssue, versionLocation);

		// Build response in expected order: code, display, issues, message, result, system, version, x-caused-by-unknown-system
		response.addParameter(createParameterComponentWithOperationOutcomeWithIssues(unknownVersionIssues.issueList()));
		response.addParameter(MESSAGE, unknownVersionIssues.messageStr());
		response.addParameter(RESULT, false);
		if (request.getCodeableConcept() == null) {
	...
	private record FallbackResolution(String hintVersion, FHIRCodeSystemVersion fallback) {}

	private FallbackResolution resolveDisplayFallbackVersion(boolean isVersionlessInclude, String codingVersion, String requestedSystem, Set<CanonicalUri> systemVersionHints, FHIRCodeValidationRequest request) {
		if (isVersionlessInclude) {
			return resolveFallbackForVersionlessInclude(requestedSystem, systemVersionHints, request);
		}
		return new FallbackResolution(null, resolveFallbackForVersionedInclude(codingVersion, requestedSystem, systemVersionHints, request));
	}

	private FHIRCodeSystemVersion resolveFallbackForVersionedInclude(String codingVersion, String requestedSystem, Set<CanonicalUri> systemVersionHints, FHIRCodeValidationRequest request) {
		...
		if (fallback == null) {
			fallback = findFallbackFromHints(requestedSystem, systemVersionHints);
		}
		...
		if (fallback == null) {
			fallback = latestVersionOrNull(requestedSystem);
		}
		return fallback;
	}

	private FallbackResolution resolveFallbackForVersionlessInclude(String requestedSystem, Set<CanonicalUri> systemVersionHints, FHIRCodeValidationRequest request) {
		...
		if (request.getForceSystemVersion() != null && request.getForceSystemVersion().getVersion() != null
			...
			hintVersion = findHintVersion(requestedSystem, systemVersionHints);
		}
		...
		return new FallbackResolution(hintVersion, fallback);
	}

	private FHIRCodeSystemVersion findFallbackFromHints(String requestedSystem, Set<CanonicalUri> systemVersionHints) {
		if (systemVersionHints == null) {
			return null;
		}
		for (CanonicalUri hint : systemVersionHints) {
			if (requestedSystem.equals(hint.getSystem()) && hint.getVersion() != null) {
				FHIRCodeSystemVersion fallback = codeSystemService.findCodeSystemVersion(
	...
	private String findHintVersion(String requestedSystem, Set<CanonicalUri> systemVersionHints) {
		if (systemVersionHints == null) {
			return null;
		}
		for (CanonicalUri hint : systemVersionHints) {
			if (requestedSystem.equals(hint.getSystem()) && hint.getVersion() != null) {
				return hint.getVersion();
			}
		}
		return null;
	}

	private FHIRCodeSystemVersion latestVersionOrNull(String requestedSystem) {
	...
	private record UnknownVersionIssues(List<OperationOutcome.OperationOutcomeIssueComponent> issueList, String messageStr) {}

	private UnknownVersionIssues buildUnknownVersionIssues(boolean isVersionlessInclude, String codingVersion, String requestedSystem, String originalIncludeVersion, String hintVersion, FHIRCodeSystemVersion fallback, String unknownMsgText, OperationOutcome.OperationOutcomeIssueComponent unknownIssue, String versionLocation) {
		if (!isVersionlessInclude) {
			return buildVersionedIncludeUnknownIssues(codingVersion, requestedSystem, originalIncludeVersion, unknownMsgText, unknownIssue, versionLocation);
		}
		return buildVersionlessIncludeUnknownIssues(codingVersion, requestedSystem, hintVersion, fallback, unknownMsgText, unknownIssue, versionLocation);
	}

	private UnknownVersionIssues buildVersionedIncludeUnknownIssues(String codingVersion, String requestedSystem, String originalIncludeVersion, String unknownMsgText, OperationOutcome.OperationOutcomeIssueComponent unknownIssue, String versionLocation) {
		...
		return new UnknownVersionIssues(issueList, messageStr);
	}

	private UnknownVersionIssues buildVersionlessIncludeUnknownIssues(String codingVersion, String requestedSystem, String hintVersion, FHIRCodeSystemVersion fallback, String unknownMsgText, OperationOutcome.OperationOutcomeIssueComponent unknownIssue, String versionLocation) {
		...
		if (codingVersion == null) {
			...
			issueList.add(unknownIssue);
			return new UnknownVersionIssues(issueList, unknownMsgText);
		}
		if (hintVersion != null) {
	...
	private Parameters buildCodeSystemNotFoundResponse(Parameters response, ValueSet hapiValueSet, String requestedSystem, String theCode, CanonicalUri missing) {
	...
	private String resolveValidatedCode(FHIRCodeValidationRequest request) {
		if (request.getCoding() != null) {
			return request.getCoding().getCode();
		}
		if (request.getCodeableConcept() != null && !request.getCodeableConcept().getCoding().isEmpty()) {
			return request.getCodeableConcept().getCoding().get(0).getCode();
		}
		return request.getCode();
	}

	private String resolveRequestedSystem(FHIRCodeValidationRequest request, CanonicalUri missing) {
		if (request.getSystem() != null && request.getSystem().getValue() != null) {
			return request.getSystem().getValue();
		}
		if (request.getCoding() != null && request.getCoding().getSystem() != null) {
			return request.getCoding().getSystem();
		}
		if (request.getCodeableConcept() != null && !request.getCodeableConcept().getCoding().isEmpty()) {
			return request.getCodeableConcept().getCoding().get(0).getSystem();
		}
		return missing.getSystem();
	}

	private String resolveCodingVersion(FHIRCodeValidationRequest request) {
		if (request.getCoding() != null) {
			return request.getCoding().getVersion();
		}
		if (request.getCodeableConcept() != null && !request.getCodeableConcept().getCoding().isEmpty()) {
			return request.getCodeableConcept().getCoding().get(0).getVersion();
		}
		return request.getSystemVersion();
	}

	private Parameters buildNoMatchingVersionResponse(Parameters response, boolean systemMatch, List<Coding> codings, Coding codingA, CodeSelectionCriteria codeSelectionCriteria, FHIRCodeValidationRequest request, CodeSystemVersionProvider codeSystemVersionProvider, Set<CanonicalUri> systemVersionHints, ValueSet hapiValueSet, List<OperationOutcome.OperationOutcomeIssueComponent> versionCheckIssues) {
		response.addParameter(RESULT, false);
		if (systemMatch) {
			return buildSystemMatchedNoVersionResponse(response, codings, codingA, codeSelectionCriteria, request, codeSystemVersionProvider, systemVersionHints, hapiValueSet, versionCheckIssues);
		}
		return buildSystemNotMatchedResponse(response, codings, codingA, request, hapiValueSet);
	}

	private Parameters buildSystemMatchedNoVersionResponse(Parameters response, List<Coding> codings, Coding codingA, CodeSelectionCriteria codeSelectionCriteria, FHIRCodeValidationRequest request, CodeSystemVersionProvider codeSystemVersionProvider, Set<CanonicalUri> systemVersionHints, ValueSet hapiValueSet, List<OperationOutcome.OperationOutcomeIssueComponent> versionCheckIssues) {
		if (codings.size() != 1) {
		...
		// comes back empty and the "version not included" message is used
		...
		return buildVersionMismatchResponse(response, codingA, codeSelectionCriteria, request, codeSystemVersionProvider, systemVersionHints, hapiValueSet, versionCheckIssues, vsVersionsForSystem, versionLocation, systemLocation, codingASystem);
	}

	private Parameters buildVersionMismatchResponse(Parameters response, Coding codingA, CodeSelectionCriteria codeSelectionCriteria, FHIRCodeValidationRequest request, CodeSystemVersionProvider codeSystemVersionProvider, Set<CanonicalUri> systemVersionHints, ValueSet hapiValueSet, List<OperationOutcome.OperationOutcomeIssueComponent> versionCheckIssues, List<FHIRCodeSystemVersion> vsVersionsForSystem, String versionLocation, String systemLocation, String codingASystem) {
		FHIRCodeSystemVersion vsVersion = vsVersionsForSystem.get(0);

		lookupDisplayForMismatch(response, codingA, vsVersion, vsVersionsForSystem, codeSelectionCriteria, request);
		...
		MismatchMessage mismatch = determineMismatchMessage(codingA, request, codeSystemVersionProvider, systemVersionHints, vsVersion, originalIncludeVersion, codingVersionExists, isVersionlessInclude, codingASystem);
		Extension mismatchExt = new Extension(HL7_SD_OUTCOME_MESSAGE_ID, new StringType(mismatch.mismatchMsgId()));
		CodeableConcept mismatchDetail = new CodeableConcept(new Coding(TX_ISSUE_TYPE, VS_INVALID, null)).setText(mismatch.mismatchMsg());
		OperationOutcome.OperationOutcomeIssueComponent mismatchIssue = createOperationOutcomeIssueComponent(
				mismatchDetail, mismatch.mismatchSeverity(), versionLocation, OperationOutcome.IssueType.INVALID,
				List.of(mismatchExt), null);

		MismatchIssueList mismatchIssueList = buildMismatchIssueList(response, codingA, mismatchIssue, mismatch.mismatchMsg(), mismatch.mismatchMsgId(), codingVersionExists, systemLocation);
		List<OperationOutcome.OperationOutcomeIssueComponent> mismatchIssues = mismatchIssueList.issues();
		String messageStr = mismatchIssueList.messageStr();
	...
	private void lookupDisplayForMismatch(Parameters response, Coding codingA, FHIRCodeSystemVersion vsVersion, List<FHIRCodeSystemVersion> vsVersionsForSystem, CodeSelectionCriteria codeSelectionCriteria, FHIRCodeValidationRequest request) {
	...
	private record MismatchMessage(String mismatchMsg, String mismatchMsgId, OperationOutcome.IssueSeverity mismatchSeverity) {}

	private MismatchMessage determineMismatchMessage(Coding codingA, FHIRCodeValidationRequest request, CodeSystemVersionProvider codeSystemVersionProvider, Set<CanonicalUri> systemVersionHints, FHIRCodeSystemVersion vsVersion, String originalIncludeVersion, boolean codingVersionExists, boolean isVersionlessInclude, String codingASystem) {
		...
		if (hintWasApplied) {
			...
			String hintVersion = determineHintVersionForMessage(request, codeSystemVersionProvider, systemVersionHints, vsVersion, codingASystem);
			String mismatchMsg = format("The code system '%s' version '%s' resulting from the version '%s' in the ValueSet include is different to the one in the value ('%s')",
					codingA.getSystem(), hintVersion,
					originalIncludeVersion != null ? originalIncludeVersion : "",
					codingA.getVersion());
			return new MismatchMessage(mismatchMsg, "VALUESET_VALUE_MISMATCH_CHANGED", OperationOutcome.IssueSeverity.ERROR);
			...
			String mismatchMsg = format("The code system '%s' version '%s' for the versionless include in the ValueSet include is different to the one in the value ('%s')",
					codingA.getSystem(), vsVersion.getVersion(), codingA.getVersion());
			return new MismatchMessage(mismatchMsg, VALUESET_VALUE_MISMATCH_DEFAULT_MSG_ID, OperationOutcome.IssueSeverity.WARNING);
		} else {
			// Normal mismatch (coding version exists but differs, or versioned include)
			String mismatchMsg = format("The code system '%s' version '%s' in the ValueSet include is different to the one in the value ('%s')",
					codingA.getSystem(), vsVersion.getVersion(), codingA.getVersion());
			return new MismatchMessage(mismatchMsg, "VALUESET_VALUE_MISMATCH", OperationOutcome.IssueSeverity.ERROR);
		}
	}

	private String determineHintVersionForMessage(FHIRCodeValidationRequest request, CodeSystemVersionProvider codeSystemVersionProvider, Set<CanonicalUri> systemVersionHints, FHIRCodeSystemVersion vsVersion, String codingASystem) {
		...
		if (request.getForceSystemVersion() != null && codingASystem.equals(request.getForceSystemVersion().getSystem())) {
			return request.getForceSystemVersion().getVersion();
			...
			return request.getCheckSystemVersion().getVersion();
		} else if (systemVersionHints != null) {
			return systemVersionHints.stream()
					.filter(h -> codingASystem.equals(h.getSystem()))
					.map(CanonicalUri::getVersion)
					.findFirst().orElse(vsVersion.getVersion());
		} else {
			return vsVersion.getVersion();
		}
	}

	private record MismatchIssueList(List<OperationOutcome.OperationOutcomeIssueComponent> issues, String messageStr) {}

	private MismatchIssueList buildMismatchIssueList(Parameters response, Coding codingA, OperationOutcome.OperationOutcomeIssueComponent mismatchIssue, String mismatchMsg, String mismatchMsgId, boolean codingVersionExists, String systemLocation) {
		...
		return new MismatchIssueList(mismatchIssues, messageStr);
	}

	private Parameters buildSystemNotMatchedResponse(Parameters response, List<Coding> codings, Coding codingA, FHIRCodeValidationRequest request, ValueSet hapiValueSet) {
		if (codings.size() != 1) {
		...
		if (Optional.ofNullable(codingA.getSystem()).orElse("").contains("ValueSet")) {
			buildValueSetReferenceIssues(response, issues, codingA, hapiValueSet);
		} else {
			String systemVersionCanonical = codingA.getSystem() + (codingA.getVersion() == null ? "" : "|" + codingA.getVersion());
			if (request.getCodeableConcept() != null) {
				buildCodeableConceptUnknownSystemIssues(response, issues, codingA, hapiValueSet, systemVersionCanonical);
			} else if (request.getCoding() != null) {
				buildCodingUnknownSystemIssues(response, issues, codingA, hapiValueSet, systemVersionCanonical);
			} else {
				buildBareCodeUnknownSystemIssues(issues, codingA, hapiValueSet);
			}
			addUnknownSystemMessage(response, codingA, request, hapiValueSet);
		}
		response.addParameter(createParameterComponentWithOperationOutcomeWithIssues(Arrays.asList(issues)));
		return response;
	}

	private void buildValueSetReferenceIssues(Parameters response, OperationOutcome.OperationOutcomeIssueComponent[] issues, Coding codingA, ValueSet hapiValueSet) {
	...
	private void buildCodeableConceptUnknownSystemIssues(Parameters response, OperationOutcome.OperationOutcomeIssueComponent[] issues, Coding codingA, ValueSet hapiValueSet, String systemVersionCanonical) {
	...
	private void buildCodingUnknownSystemIssues(Parameters response, OperationOutcome.OperationOutcomeIssueComponent[] issues, Coding codingA, ValueSet hapiValueSet, String systemVersionCanonical) {
		CodeableConcept details1 = new CodeableConcept(new Coding(TX_ISSUE_TYPE, NOT_IN_VS, null)).setText(format(CODE_NOT_IN_VS, createFullyQualifiedCodeString(codingA), CanonicalUri.of(hapiValueSet.getUrl(), hapiValueSet.getVersion())));
		issues[0] = createOperationOutcomeIssueComponent(details1, OperationOutcome.IssueSeverity.ERROR, OUTCOME_CODING_CODE, OperationOutcome.IssueType.CODEINVALID, null, null);
	...
	private void buildBareCodeUnknownSystemIssues(OperationOutcome.OperationOutcomeIssueComponent[] issues, Coding codingA, ValueSet hapiValueSet) {
		CodeableConcept details1 = new CodeableConcept(new Coding(TX_ISSUE_TYPE, NOT_IN_VS, null)).setText(format(CODE_NOT_IN_VS, createFullyQualifiedCodeString(codingA), CanonicalUri.of(hapiValueSet.getUrl(), hapiValueSet.getVersion())));
	...
	private void addUnknownSystemMessage(Parameters response, Coding codingA, FHIRCodeValidationRequest request, ValueSet hapiValueSet) {
		if(codingA.getSystem()==null){
	...
	private void handleConceptFound(int i, List<Coding> codings, Coding codingA, FHIRConcept concept, Parameters response, List<OperationOutcome.OperationOutcomeIssueComponent> issues, Set<FHIRCodeSystemVersion> resolvedCodeSystemVersionsMatchingCodings, CodeSelectionCriteria codeSelectionCriteria, List<LanguageDialect> languageDialects, FHIRCodeValidationRequest request, ValueSet hapiValueSet) {
		...
		addDeprecatedInValueSetWarning(codingA, issues, hapiValueSet);
		addNormalizedCodeIssue(codingA, concept, response, issues, resolvedCodeSystemVersionsMatchingCodings);
		addStatusCheckWarnings(issues, resolvedCodeSystemVersionsMatchingCodings, codeSelectionCriteria);

		String codingADisplay = codingA.getDisplay();
		if (codingADisplay == null || Objects.equals(codingADisplay, concept.getDisplay())) {
			validateMatchingDisplay(codingA, concept, response, issues, languageDialects, request);
		} else {
			validateDisplayMismatch(i, codingA, concept, response, issues, languageDialects, request, hapiValueSet);
		}
	}
	...
	private boolean handleInactiveConcept(Coding codingA, FHIRConcept concept, Parameters response, List<OperationOutcome.OperationOutcomeIssueComponent> issues, FHIRCodeValidationRequest request, ValueSet hapiValueSet) {
		if (FHIRHelper.isSnomedUri(codingA.getSystem()) && !concept.isActive()) {
			...
			return handleNonSnomedInactiveConcept(codingA, concept, response, issues, request, hapiValueSet);
		}
		return false;
	}

	private boolean handleNonSnomedInactiveConcept(Coding codingA, FHIRConcept concept, Parameters response, List<OperationOutcome.OperationOutcomeIssueComponent> issues, FHIRCodeValidationRequest request, ValueSet hapiValueSet) {
		response.addParameter("inactive", true);
	...
	private void addDeprecatedInValueSetWarning(Coding codingA, List<OperationOutcome.OperationOutcomeIssueComponent> issues, ValueSet hapiValueSet) {
	...
	private void addNormalizedCodeIssue(Coding codingA, FHIRConcept concept, Parameters response, List<OperationOutcome.OperationOutcomeIssueComponent> issues, Set<FHIRCodeSystemVersion> resolvedCodeSystemVersionsMatchingCodings) {
	...
	private void addStatusCheckWarnings(List<OperationOutcome.OperationOutcomeIssueComponent> issues, Set<FHIRCodeSystemVersion> resolvedCodeSystemVersionsMatchingCodings, CodeSelectionCriteria codeSelectionCriteria) {
	...
	private void validateMatchingDisplay(Coding codingA, FHIRConcept concept, Parameters response, List<OperationOutcome.OperationOutcomeIssueComponent> issues, List<LanguageDialect> languageDialects, FHIRCodeValidationRequest request) {
	...
	private void validateDisplayMismatch(int i, Coding codingA, FHIRConcept concept, Parameters response, List<OperationOutcome.OperationOutcomeIssueComponent> issues, List<LanguageDialect> languageDialects, FHIRCodeValidationRequest request, ValueSet hapiValueSet) {
		String codingADisplay = codingA.getDisplay();
		DesignationMatch designationMatch = findDisplayDesignationMatch(codingADisplay, concept, response, languageDialects, request, hapiValueSet);
		if(!designationMatch.resultOk()) {
			reportWrongDisplay(i, codingA, concept, designationMatch.termMatch(), response, issues, request);
		}
	}

	private record DesignationMatch(FHIRDesignation termMatch, boolean resultOk) {}

	private DesignationMatch findDisplayDesignationMatch(String codingADisplay, FHIRConcept concept, Parameters response, List<LanguageDialect> languageDialects, FHIRCodeValidationRequest request, ValueSet hapiValueSet) {
		...
		for (int j = 0; j < designations.size() && !resultOk; j++) {
			...
			if (codingADisplay.equalsIgnoreCase(designation.getValue())) {
				...
				if (languageMatchesDialects(designationLanguage, languageDialects)) {
					setResultTrueIfNotFalseAlready(response);
					response.addParameter(DISPLAY, termMatch.getValue());
					resultOk = true;
				} else if (isDesignationLanguageMismatch(designationLanguage, languageDialects, request, hapiValueSet)) {
					termMatch = null;
		...
		return new DesignationMatch(termMatch, resultOk);
	}

	private boolean languageMatchesDialects(String designationLanguage, List<LanguageDialect> languageDialects) {
		return designationLanguage == null || languageDialects.stream()
				.anyMatch(languageDialect -> designationLanguage.equals(languageDialect.getLanguageCode()));
	}

	private boolean isDesignationLanguageMismatch(String designationLanguage, List<LanguageDialect> languageDialects, FHIRCodeValidationRequest request, ValueSet hapiValueSet) {
		return languageDialects.isEmpty() && !LANG_EN.equals(designationLanguage) && (
	...
	private void reportWrongDisplay(int i, Coding codingA, FHIRConcept concept, FHIRDesignation termMatch, Parameters response, List<OperationOutcome.OperationOutcomeIssueComponent> issues, FHIRCodeValidationRequest request) {
		...
		if (termMatch != null) {
			...
			reportWrongDisplayNoMatch(codingA, concept, response, issues, severity, locationExpression, request);
		}
	}

	private void reportWrongDisplayNoMatch(Coding codingA, FHIRConcept concept, Parameters response, List<OperationOutcome.OperationOutcomeIssueComponent> issues, OperationOutcome.IssueSeverity severity, String locationExpression, FHIRCodeValidationRequest request) {
		SelectedDisplay selectedDisplay = selectDisplay(codingA.getSystem(), request.getDisplayLanguage(), concept);
		response.addParameter(DISPLAY, selectedDisplay.selectedDisplayValue);
		CodeableConcept cc;
		if(selectedDisplay.languageAvailable == null) {
			...
			cc = buildMultiLanguageWrongDisplay(codingA, concept, response, selectedDisplay, request);
		} else {
	...
	private CodeableConcept buildMultiLanguageWrongDisplay(Coding codingA, FHIRConcept concept, Parameters response, SelectedDisplay selectedDisplay, FHIRCodeValidationRequest request) {
	...
	private record NotFoundResult(String locationExpression, String message) {}

	private void handleConceptNotFound(int i, List<Coding> codings, Coding codingA, Parameters response, List<OperationOutcome.OperationOutcomeIssueComponent> issues, Set<FHIRCodeSystemVersion> resolvedCodeSystemVersionsMatchingCodings, FHIRCodeValidationRequest request, ValueSet hapiValueSet) {
		response.setParameter(RESULT, false);
		NotFoundResult result;
		if (request.getCodeableConcept() != null) {
			result = handleCodeableConceptNotFound(i, codings, codingA, response, issues, resolvedCodeSystemVersionsMatchingCodings, request, hapiValueSet);
		} else if (request.getCoding() != null) {
			result = handleCodingNotFound(codingA, issues, resolvedCodeSystemVersionsMatchingCodings, request, hapiValueSet);
		} else {
			result = handleBareCodeNotFound(codingA, response, issues, resolvedCodeSystemVersionsMatchingCodings, request, hapiValueSet);
		}
		response.setParameter(MESSAGE, result.message());

		if(request.getInferSystem() != null && request.getInferSystem().booleanValue()){
			issues.add(createOperationOutcomeIssueComponent(new CodeableConcept().addCoding(new Coding(TX_ISSUE_TYPE, "cannot-infer",null)).setText(result.message()), OperationOutcome.IssueSeverity.ERROR, result.locationExpression(), OperationOutcome.IssueType.NOTFOUND, null , null));
			List<Parameters.ParametersParameterComponent> systemParameters = new ArrayList<>(response.getParameters(SYSTEM));
			systemParameters.forEach(v -> response.removeChild(PARAMETER, v));
		}
	}

	private NotFoundResult handleCodeableConceptNotFound(int i, List<Coding> codings, Coding codingA, Parameters response, List<OperationOutcome.OperationOutcomeIssueComponent> issues, Set<FHIRCodeSystemVersion> resolvedCodeSystemVersionsMatchingCodings, FHIRCodeValidationRequest request, ValueSet hapiValueSet) {
		List<Parameters.ParametersParameterComponent> codeParameters = new ArrayList<>(response.getParameters(CODE));
	...
	private NotFoundResult handleCodingNotFound(Coding codingA, List<OperationOutcome.OperationOutcomeIssueComponent> issues, Set<FHIRCodeSystemVersion> resolvedCodeSystemVersionsMatchingCodings, FHIRCodeValidationRequest request, ValueSet hapiValueSet) {
		...
		if(resolvedCodeSystemVersionsMatchingCodings.size() == 1 && codeSystemIncludesConcept(resolvedCodeSystemVersionsMatchingCodings.iterator().next(), codingA)) {
			...
			if (providedDisplay != null && !providedDisplay.isEmpty()) {
				message = appendCodingDisplayMismatch(codingA, issues, resolvedCodeSystemVersionsMatchingCodings, request, message, providedDisplay);
			}
		} else {
	...
	private String appendCodingDisplayMismatch(Coding codingA, List<OperationOutcome.OperationOutcomeIssueComponent> issues, Set<FHIRCodeSystemVersion> resolvedCodeSystemVersionsMatchingCodings, FHIRCodeValidationRequest request, String message, String providedDisplay) {
		...
		if (csConcept != null && csConcept.getDisplay() != null && !providedDisplay.equalsIgnoreCase(csConcept.getDisplay())) {
			...
			return message + "; " + displayMsg;
		}
		return message;
	}

	private NotFoundResult handleBareCodeNotFound(Coding codingA, Parameters response, List<OperationOutcome.OperationOutcomeIssueComponent> issues, Set<FHIRCodeSystemVersion> resolvedCodeSystemVersionsMatchingCodings, FHIRCodeValidationRequest request, ValueSet hapiValueSet) {
		String locationExpression = CODE;
		String message;
		...
		if (isSnomedPostCoordinated && !resolvedCodeSystemVersionsMatchingCodings.isEmpty()) {
			...
			message = buildBareCodeNotInCsMessage(codingA, response, issues, resolvedCodeSystemVersionsMatchingCodings, request, hapiValueSet, locationExpression);
		}
		return new NotFoundResult(locationExpression, message);
	}

	private String buildBareCodeNotInCsMessage(Coding codingA, Parameters response, List<OperationOutcome.OperationOutcomeIssueComponent> issues, Set<FHIRCodeSystemVersion> resolvedCodeSystemVersionsMatchingCodings, FHIRCodeValidationRequest request, ValueSet hapiValueSet, String locationExpression) {
		FHIRCodeSystemVersion csVersion = resolvedCodeSystemVersionsMatchingCodings.iterator().next();
		String csConceptCode = codingA.getCode();
		...
		if (conceptExistsInCS) {
			return buildBareCodeFoundInCsMessage(codingA, response, issues, csVersion, conceptDisplay, hapiValueSet, locationExpression);
		}
		return buildBareCodeUnknownMessage(codingA, response, issues, request, hapiValueSet, locationExpression);
	}

	private String buildBareCodeFoundInCsMessage(Coding codingA, Parameters response, List<OperationOutcome.OperationOutcomeIssueComponent> issues, FHIRCodeSystemVersion csVersion, String conceptDisplay, ValueSet hapiValueSet, String locationExpression) {
		...
		String codeWithDisplay = codingA.getDisplay() != null && !codingA.getDisplay().isEmpty()
				? codingA.getCode() + " ('" + codingA.getDisplay() + "')"
				: codingA.getCode();
		String message = "The provided code '" + codingA.getSystem() + "#" + codeWithDisplay + "' was not found in the value set '" + CanonicalUri.of(hapiValueSet.getUrl(), hapiValueSet.getVersion()) + "'";
	...
	private String buildBareCodeUnknownMessage(Coding codingA, Parameters response, List<OperationOutcome.OperationOutcomeIssueComponent> issues, FHIRCodeValidationRequest request, ValueSet hapiValueSet, String locationExpression) {
		// Concept not found in the CS - report as unknown code
		if (codingA.getDisplay() != null && !codingA.getDisplay().isEmpty()) {
			response.addParameter(DISPLAY, codingA.getDisplay());
		}
		String message = format(SYSTEM_CODE_NOT_IN_VS, codingA.getSystem(), codingA.getCode(), CanonicalUri.of(hapiValueSet.getUrl(), hapiValueSet.getVersion()));
		OperationOutcome.OperationOutcomeIssueComponent notInVsIssue = createOperationOutcomeIssueComponent(
				new CodeableConcept().addCoding(new Coding(TX_ISSUE_TYPE, NOT_IN_VS, null)).setText(message),
				OperationOutcome.IssueSeverity.ERROR, null, OperationOutcome.IssueType.CODEINVALID, null, null);
		notInVsIssue.addExpression(locationExpression);
