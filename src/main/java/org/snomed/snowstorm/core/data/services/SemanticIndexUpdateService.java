...
public class SemanticIndexUpdateService extends ComponentService implements CommitListener {
	...
	private Set<Long> buildRelevantPartsOfExistingGraph(GraphBuilder graphBuilder, Form form,
		...
		try (final SearchHitsIterator<Relationship> changedIsARelationships = elasticsearchOperations.searchForStream(new NativeQueryBuilder()
				...
				.withSourceFilter(new FetchSourceFilter(null, new String[]{Relationship.Fields.SOURCE_ID, Relationship.Fields.DESTINATION_ID}, null))
				.withPageable(LARGE_PAGE).build(), Relationship.class)) {
		...
		if (form.isStated()) {
			...
			try (final SearchHitsIterator<ReferenceSetMember> changedAxioms = elasticsearchOperations.searchForStream(new NativeQueryBuilder()
					...
					.withSourceFilter(new FetchSourceFilter(null, new String[]{ReferenceSetMember.Fields.REFERENCED_COMPONENT_ID, ReferenceSetMember.OwlExpressionFields.OWL_EXPRESSION_FIELD_PATH}, null))
		...
		NativeQuery query = new NativeQueryBuilder()
				...
				.withSourceFilter(new FetchSourceFilter(null, new String[]{QueryConcept.Fields.ANCESTORS}, null))
	...
	private void buildGraphFromExistingNodes(Set<Long> nodesToLoad, boolean stated, GraphBuilder graphBuilder, BranchCriteria branchCriteriaForAlreadyCommittedContent,
		...
		NativeQueryBuilder queryConceptQuery = new NativeQueryBuilder()
				...
				.withSourceFilter(new FetchSourceFilter(null, new String[]{QueryConcept.Fields.CONCEPT_ID, QueryConcept.Fields.PARENTS, QueryConcept.Fields.ANCESTORS}, null))
				.withPageable(LARGE_PAGE);
