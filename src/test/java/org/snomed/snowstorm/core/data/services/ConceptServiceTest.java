...
public class ConceptServiceTest extends AbstractTest {
	...
	public void testSaveConceptWithAxioms() throws ServiceException {
		...
		Assert.assertNotNull(savedConcept);
		assertEquals(1, savedConcept.getAdditionalAxioms().size());
		assertEquals(1, savedConcept.getGciAxioms().size());
		Axiom axiom = savedConcept.getAdditionalAxioms().iterator().next();
...
//	@Test
	public void testCreateUpdate10KConcepts() throws ServiceException {
		...
		Page<ConceptMini> conceptDescendants = queryService.findDescendantsAsConceptMinis(SNOMEDCT_ROOT, "MAIN/A", Relationship.CharacteristicType.stated, PageRequest.of(0, 50), conceptService);
